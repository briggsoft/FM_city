...
//--------------
   ...
   float2 ShadowOffSet2x2[5];
   float2 ShadowOffSet3x3[9];
   float2 ShadowOffSet4x4[16];
   float ProjShift;
...
//--------------
   ...
   float4 PS_ShadowMap_1(OutPut IN) : COLOR
	...
	float3 WNormals=tex2Dproj(WorldNormals,IN.Proj)*2-1;
     	float4 ProjVec=mul(ProjMat,mul(LightV,SpotProjMatrix)); 
	float2 ShadowVec=(ProjVec.xy/ProjVec.z)+((dot(WNormals,1)+tex2D(JitterMap,IN.JitterUV))*ShadowOffSet);
        float ShadowMap=(tex2D(DepthMap,ShadowVec) > Depth) ? 1.0 : 0.0;
	LightV=normalize(-LightV);
	float Normal=saturate(dot(WNormals,LightV));
	float3 Projectlight=tex2Dproj(Project,mul(ProjMat,mul(LightV,LightProjMatrix)))*saturate(ProjVec.z);
	return float4(Normal*LightColor*Projectlight*(1-pow(Depth,Attenuation))*ShadowMap,1);
     } 
   float4 PS_ShadowMap_2(OutPut IN) : COLOR
	...
	float3 WNormals=tex2Dproj(WorldNormals,IN.Proj)*2-1;
     	float4 ProjVec=mul(ProjMat,mul(LightV,SpotProjMatrix)); 
	float2 ShadowVec=(ProjVec.xy/ProjVec.z)+((dot(WNormals,1)+tex2D(JitterMap,IN.JitterUV))*ShadowOffSet);
	float ShadowMap=((tex2D(DepthMap,ShadowVec+ShadowOffSet2x2[0]) > Depth) ? 0.2 : 0.0)
     		       +((tex2D(DepthMap,ShadowVec+ShadowOffSet2x2[1]) > Depth) ? 0.2 : 0.0)
     		       +((tex2D(DepthMap,ShadowVec+ShadowOffSet2x2[2]) > Depth) ? 0.2 : 0.0)
     		       +((tex2D(DepthMap,ShadowVec+ShadowOffSet2x2[3]) > Depth) ? 0.2 : 0.0)
     		       +((tex2D(DepthMap,ShadowVec+ShadowOffSet2x2[4]) > Depth) ? 0.2 : 0.0);
	LightV=normalize(-LightV);
	float Normal=saturate(dot(WNormals,LightV));
	float3 Projectlight=tex2Dproj(Project,mul(ProjMat,mul(LightV,LightProjMatrix)))*saturate(ProjVec.z);
	return float4(Normal*LightColor*Projectlight*(1-pow(Depth,Attenuation))*ShadowMap,1);
   ...
   float4 PS_ShadowMap_3(OutPut IN) : COLOR
	...
	float3 WNormals=tex2Dproj(WorldNormals,IN.Proj)*2-1;
     	float4 ProjVec=mul(ProjMat,mul(LightV,SpotProjMatrix)); 
	float2 ShadowVec=(ProjVec.xy/ProjVec.z)+((dot(WNormals,1)+tex2D(JitterMap,IN.JitterUV))*ShadowOffSet);
	float ShadowMap=((tex2D(DepthMap,ShadowVec+ShadowOffSet3x3[0]) > Depth) ? 0.111111111 : 0.0)
                       +((tex2D(DepthMap,ShadowVec+ShadowOffSet3x3[1]) > Depth) ? 0.111111111 : 0.0)
                       +((tex2D(DepthMap,ShadowVec+ShadowOffSet3x3[2]) > Depth) ? 0.111111111 : 0.0)
                       +((tex2D(DepthMap,ShadowVec+ShadowOffSet3x3[3]) > Depth) ? 0.111111111 : 0.0)
                       +((tex2D(DepthMap,ShadowVec+ShadowOffSet3x3[4]) > Depth) ? 0.111111111 : 0.0)
                       +((tex2D(DepthMap,ShadowVec+ShadowOffSet3x3[5]) > Depth) ? 0.111111111 : 0.0)
                       +((tex2D(DepthMap,ShadowVec+ShadowOffSet3x3[6]) > Depth) ? 0.111111111 : 0.0)
                       +((tex2D(DepthMap,ShadowVec+ShadowOffSet3x3[7]) > Depth) ? 0.111111111 : 0.0)
                       +((tex2D(DepthMap,ShadowVec+ShadowOffSet3x3[8]) > Depth) ? 0.111111111 : 0.0);
	LightV=normalize(-LightV);
	float Normal=saturate(dot(WNormals,LightV));
	float3 Projectlight=tex2Dproj(Project,mul(ProjMat,mul(LightV,LightProjMatrix)))*saturate(ProjVec.z);
	return float4(Normal*LightColor*Projectlight*(1-pow(Depth,Attenuation))*ShadowMap,1);
   ...
   float4 PS_ShadowMap_4(OutPut IN) : COLOR
	...
	float3 WNormals=tex2Dproj(WorldNormals,IN.Proj)*2-1;
     	float4 ProjVec=mul(ProjMat,mul(LightV,SpotProjMatrix)); 
	float2 ShadowVec=(ProjVec.xy/ProjVec.z)+((dot(WNormals,1)+tex2D(JitterMap,IN.JitterUV))*ShadowOffSet);
	float ShadowMap=((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[0]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[1]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[2]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[3]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[4]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[5]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[6]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[7]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[8]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[9]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[10]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[11]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[12]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[13]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[14]) > Depth) ? 0.0625 : 0.0)
	               +((tex2D(DepthMap,ShadowVec+ShadowOffSet4x4[15]) > Depth) ? 0.0625 : 0.0);
	LightV=normalize(-LightV);
	float Normal=saturate(dot(WNormals,LightV));
	float3 Projectlight=tex2Dproj(Project,mul(ProjMat,mul(LightV,LightProjMatrix)))*saturate(ProjVec.z);
	return float4(Normal*LightColor*Projectlight*(1-pow(Depth,Attenuation))*ShadowMap,1);
